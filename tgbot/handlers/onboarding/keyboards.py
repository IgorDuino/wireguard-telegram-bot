...
def choose_device_pc() -> InlineKeyboardMarkup:
    buttons = [
        ...
        [
            InlineKeyboardButton(
                "🖥️ Windows", callback_data=f"choose_device:windows:{rand_suffix()}"
            )
        ],
        [
            InlineKeyboardButton(
                "🍏 MacOS", callback_data=f"choose_device:macos:{rand_suffix()}"
            ),
            InlineKeyboardButton(
                "🐧 Linux", callback_data=f"choose_device:linux:{rand_suffix()}"
            ),
        ],
    ]
...
def main_menu(user: User) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                "💳 Оплатить", callback_data=f"choose_pay_profile:{rand_suffix()}"
            )
        ],
        [
            InlineKeyboardButton(
    ...
    if profile_server_id:
        buttons[1] = [
            InlineKeyboardButton(
                "💳 Оплатить",
                callback_data=f"choose_pay_period:{profile_server_id}:{rand_suffix()}",
            )
        ]
    return InlineKeyboardMarkup(buttons)
...
def profiles_menu(user: User) -> InlineKeyboardMarkup:
    ...
    for profile in profiles:
        buttons.append(
            [
                InlineKeyboardButton(
                    f"{profile.name} - оплачен до {datetime.strftime(profile.active_until, '%d.%m.%Y')}",
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                "🔙 Главное меню", callback_data=f"main_menu:{rand_suffix()}"
            )
        ]
    )
    return InlineKeyboardMarkup(buttons)
...
def profile_menu(profile: VPNProfile) -> InlineKeyboardMarkup:
    buttons = [
        ...
        [
            InlineKeyboardButton(
                "📝 Инструкция по подключению", url=TELEGRAPH_INSTRUCTION_LINK
            )
        ],
        [
            InlineKeyboardButton(
                "🟥 Отказаться от профиля",
...
def choose_pay_profile_handler(profiles: List[VPNProfile]) -> InlineKeyboardMarkup:
    ...
    for profile in profiles:
        buttons.append(
            [
                InlineKeyboardButton(
                    f"{profile.name} - Оплачен до {profile.active_until}",
...
def choose_pay_period(profile_server_id) -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                "1 месяц", callback_data=f"pay:{profile_server_id}:1:{rand_suffix()}"
            ),
            InlineKeyboardButton(
                "3 месяца", callback_data=f"pay:{profile_server_id}:3:{rand_suffix()}"
            ),
            InlineKeyboardButton(
                "6 месяцев", callback_data=f"pay:{profile_server_id}:6:{rand_suffix()}"
            ),
        ],
        [
            InlineKeyboardButton(
...
def pay_button(profile: VPNProfile, period: int) -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                f"💳 Оплатить профиль {profile.name} на {period} дней",
                web_app=WebAppInfo(
