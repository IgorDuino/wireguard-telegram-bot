...
def command_clear(update: Update, context: CallbackContext) -> None:
    ...
    msg = update.message.reply_text(
        text="Clearing keyboard", reply_markup=ReplyKeyboardRemove()
    )
...
def choose_device_handler(update: Update, context: CallbackContext) -> None:
    ...
    if len(servers) == 0:
        ...
        context.bot.send_message(chat_id=ROOT_ADMIN_ID, text=f"No servers")
        return
    ...
    for server in servers:
        try:
            ...
            context.bot.send_message(
                chat_id=ROOT_ADMIN_ID, text=f"Wireguard server {server} auth error"
            )
        ...
        update.callback_query.edit_message_text(text=shop_text.no_available_servers)
        context.bot.send_message(
            chat_id=ROOT_ADMIN_ID, text=f"All servers are unavailable"
        )
        return
    ...
    new_profile.active_until = new_profile.created_at + timedelta(
    ...
    context.bot.delete_message(
        chat_id=user_id, message_id=update.callback_query.message.message_id
    )

    context.bot.send_photo(chat_id=user_id, photo=qr_code)

    context.bot.send_document(chat_id=user_id, document=config, filename=f"{name}.conf")
...
def profile_handler(update: Update, context: CallbackContext) -> None:
    ...
    update.callback_query.edit_message_text(
        f"Профиль {profile.name}", reply_markup=keyboards.profile_menu(profile)
    )
...
def choose_pay_period_handler(update: Update, context: CallbackContext) -> None:
    ...
    if profile:
        update.callback_query.edit_message_text(
            text=shop_text.choose_pay_period_text,
...
def choose_pay_profile_handler(update: Update, context: CallbackContext) -> None:
    ...
    if not profiles:
        ...
        return
    update.callback_query.edit_message_text(
        text=shop_text.choose_profile_text,
...
def pay_handler(update: Update, context: CallbackContext) -> None:
    ...
    update.callback_query.edit_message_text(
        text=shop_text.pay_text, reply_markup=keyboards.pay_button(profile, period)
    )
...
def new_profile_handler(update: Update, context: CallbackContext) -> None:
    user = User.get_user(update, context)
    if user.is_trial:
        if VPNProfile.objects.filter(user=user).count() > 0:
            update.callback_query.edit_message_text(
